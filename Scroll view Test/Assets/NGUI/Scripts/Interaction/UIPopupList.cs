	...
	public virtual void Show ()
	{
		if (enabled && NGUITools.GetActive(gameObject) && mChild == null && isValid && items.Count > 0)
		{
			...
			for (int i = 0, imax = items.Count; i < imax; ++i)
			{
                ...
                lbl.bitmapFont = (Resources.Load("Font/BM-JUA", typeof(GameObject)) as GameObject).GetComponent<UIFont>();
			...
			TopDown_AlignRight(); //톱다운 리스트 정렬 위치 변경
		}
		else OnSelect(false);
	}

	//톱다운 리스트 정렬 위치 변경 
	public void TopDown_AlignRight()
	{
		//상단 스프라이트의 크기에서 현재 다운 박스의 크기를 계산한다.
		int PopupBox_w, DownBox_w;

		PopupBox_w = transform.GetChild(0).GetComponent<UISprite>().width;
		DownBox_w = mChild.transform.GetChild(0).GetComponent<UISprite>().width;

		//다운된 리스트의 크기가 원래 팝업 박스의 크기보다 커지면 리스트의 위치를 왼쪽으로 이동 식킨다.
		if (DownBox_w > PopupBox_w)
		{
			mChild.transform.localPosition = new Vector3(mChild.transform.localPosition.x - (DownBox_w - PopupBox_w), mChild.transform.localPosition.y, mChild.transform.localPosition.z);
		}
